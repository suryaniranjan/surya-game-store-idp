...
#   aws_apigatewayv2_stage.default now includes:
#     - access_log_settings block pointing to the CloudWatch log group
#       created in cloudwatch.tf
#     - depends_on the log group so the group exists before the stage deploys
#   No other changes. All business logic, routes, integrations are untouched.
# ─────────────────────────────────────────────────────────────────────────────
...
locals {
  ...
  lambdas = {
    ...
    payment = {
      ...
      description = "Payment service (simulated Razorpay)"
      env_vars = {
        ORDERS_TABLE   = aws_dynamodb_table.orders_table.name
        CART_API_URL   = var.cart_api_url
        PRODUCTS_TABLE = local.dynamodb_tables.products.name
  ...
  api_routes = {
    ...
    # Payment service
    "POST /v1/payment/initiate" = { lambda_key = "payment" }
    "POST /v1/payment/confirm"  = { lambda_key = "payment" }
    "POST /v1/payment/fail"     = { lambda_key = "payment" }
  }
...
resource "aws_iam_role_policy" "lambda_payment_dynamodb" {
  name = "${var.project_name}-lambda-payment-policy"
  role = aws_iam_role.lambda_exec.id
...
resource "aws_lambda_function" "functions" {
  ...
  # Ensure log group exists before function is created/updated
  # so the first invocation log stream lands in a managed group
...
# ─────────────────────────────────────────────────────────────────────────────
# CHANGE: aws_apigatewayv2_stage.default
#
# Added:
#   access_log_settings — writes structured JSON access logs to the
#   CloudWatch log group created in cloudwatch.tf.
#   The $context variables capture method, route, status, latency,
#   request ID, and source IP for every API call.
#
#   depends_on — ensures the log group and the account-level CloudWatch
#   role (aws_api_gateway_account.main) exist before the stage is created.
#
# Unchanged: name, auto_deploy, tags.
# ─────────────────────────────────────────────────────────────────────────────
...
resource "aws_apigatewayv2_stage" "default" {
  ...
  depends_on = [
    aws_cloudwatch_log_group.api_gw_access_logs,

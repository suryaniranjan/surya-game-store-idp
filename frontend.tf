...
resource "aws_cloudfront_distribution" "frontend" {
  ...
  origin {
    ...
    origin_id                = "S3-images-${var.images_bucket_name}"
    origin_access_control_id = aws_cloudfront_origin_access_control.images.id
  }

  ordered_cache_behavior {
  path_pattern           = "/products/*"   # was /images/*
  allowed_methods        = ["GET", "HEAD"]
  cached_methods         = ["GET", "HEAD"]
  target_origin_id       = "S3-images-${var.images_bucket_name}"
...
# ── Reference the existing images bucket (already created outside Terraform) ──
